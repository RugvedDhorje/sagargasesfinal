...
import { Cylinder_List } from '../assets/assets';


const CylinderDetail = () => {
  const { cylinder_Id } = useParams();
  const cylinderId = Number(cylinder_Id);
  const cylinder = Cylinder_List.find((c) => c.cylinder_Id === cylinderId);
    ...
    <div>
      ...
      <div className="w-11/12 md:w-10/12 mx-auto my-10 flex flex-col md:flex-row max-w-screen-2xl">
        <div className="md:w-8/12 md:mr-16 text-[#3d4351]">
          <p className="text-[16px]">{cylinder.description}</p>
          <div className="border border-1 border-slate-200 rounded-md my-5">
            <motion.h2
              ...
              className="py-3 font-semibold text-[16px] px-4 text-[#0156d7] w-11/12 mx-auto "
            >
              Applications
            </motion.h2>
            <ul className="px-4 w-11/12 mx-auto list-disc custom-list list-inside text-left text-[14px] sm:text-[16px] mb-5">
              {cylinder.application.map((point, index) => (
          ...
          <div>
            ...
            <p className="text-[16px]">{cylinder.supply}</p>
          </div>
        </div>
        <motion.div
          initial={{ scale: 0.9 }}
        ...
        >
          <img src={cylinder.img} alt="product image" className="rounded-lg mx-auto md:mx-0" />
        </motion.div>
      </div>
      <div className="w-11/12 md:w-10/12 mx-auto max-w-screen-2xl bg-slate-100 flex flex-col md:flex-row justify-center gap-4 items-center rounded-xl py-4 mb-10">
        <h2 className="text-[24px] md:text-[30px] text-[#003973] font-light text-center">
          Discover How {cylinder.name} Can Improve Your Process
        </h2>
